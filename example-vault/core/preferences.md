...
the file to `vault/_for-deletion/` instead of deleting. It's safer (I can
move it back), tidier (every tool excludes that folder), and works the same
way on every host. I empty the folder myself periodically, usually as part
of a sweep.
